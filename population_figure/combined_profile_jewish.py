#!/usr/bin/env python3
"""
combined_profile_jewish.py
2020 vs 2040 density profile – Jewish areas only (sector: Jewish + U. Orthodox).

Density denominator = area of Jewish statistical-area footprint within each ring,
not the full donut, so the profile reflects actual Jewish neighbourhood density.
"""
...
OUT_PATH = os.path.join(HERE, "jerusalem_density_profile_2020_2040_jewish.png")
...
# ── Load – Jewish areas only ──────────────────────────────────────────────────
print("Loading jer_areas (Jewish sectors) …")
areas_all = gpd.read_file(SHP_PATH)
areas_all = areas_all[areas_all["in_jeru"] == 1].copy()
areas = areas_all[areas_all["sector"].isin(JEWISH_SECTORS)].copy()
areas["area_m2"] = areas.geometry.area
print(f"  Jewish areas: {len(areas)}  "
      f"(excluded {len(areas_all) - len(areas)} Arab/other areas)")
...
# Intersection fraction (share of each area's geometry inside each ring)
inter_frac_rings = [
    areas.geometry.intersection(g).area / areas["area_m2"].replace(0, np.nan)
    for g in ring_geoms
]

# Jewish footprint area within each ring (km²) – used as density denominator
jewish_ring_area_km2 = [
    areas.geometry.intersection(g).area.sum() / 1e6
    for g in ring_geoms
]

# For total-population label: Jewish footprint within 16 km
inter_frac_16 = (
...
for cfg in YEARS:
    ...
    densities = []
    for frac, j_area in zip(inter_frac_rings, jewish_ring_area_km2):
        pop_in_ring = (frac * pop).fillna(0).sum()
        # Avoid division by zero for rings with no Jewish footprint
        densities.append(pop_in_ring / j_area if j_area > 0 else 0)
    ...
    y   = np.array(list(reversed(y_r)) + [densities[0]] + y_r)
    ...
    for r, d in enumerate(densities):
        print(f"  {year}  Ring {r*2}-{r*2+2} km: {d:,.0f} p/km²  "
              f"(Jewish footprint {jewish_ring_area_km2[r]:.1f} km²)")
    print(f"  {year}  Total within 16 km: {total_pop:,.0f}\n")
...
note = (
    "Methodology\n"
    "Jewish areas only (sector: Jewish +\n"
    "U. Orthodox). Population split by\n"
    "geometric intersection share per ring.\n"
    "Density denominator = Jewish area\n"
    "footprint within each ring (km²).\n\n"
